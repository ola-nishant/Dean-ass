...
public:
    ...
    /* ****************************************************************************** */
    T& operator[] (int i) const{ //Access by index (zero based)
        if (i > -1 && i < S) return D[i];
        throw std::out_of_range("myArray: Out of range access\n");
    }

    myArray<T>& operator = (myArray<T> x){ //asignment
        if (x.Size() <= 0) throw std::out_of_range("myArray: Assigning non initialized array\n");
        Clear();
        Allocate(x.S);
    ...
    myArray<T>& operator - () { //unary -
        if (Size() <= 0) throw std::out_of_range("myArray: negative of unassigned array\n");
        myArray<T> x(*this);
    ...
    myArray<T>& operator += (myArray<T> x){ //add + assignement
        if (x.Size() != Size()) throw std::out_of_range("myArray +: Different sized array\n");
        for (int i = 0; i < x.Size(); i++) D[i] += x[i];
        return *this;
    }

    friend myArray<T> operator + (myArray<T> x, myArray<T> y){ //addition
        if (x.Size() != y.Size()) throw std::out_of_range("myArray +: Different sized array\n");
        myArray<T> z(x.Size());
    ...
    friend myArray<T> operator * (myArray<T> a, T x){ //multiply by a constant
        if (a.Size() <= 0) throw std::out_of_range("myArray: negative of unassigned array\n");
        myArray<T> y(a);
